...
df_active = pd.read_excel(
    "Active-Inactive-LEO-JailOfficer 1-29-2026 Settlement Production.xlsx",
    sheet_name="Active",
)
df_inactive = pd.read_excel(
    "Active-Inactive-LEO-JailOfficer 1-29-2026 Settlement Production.xlsx",
    sheet_name="Inactive",
)
...
df_merged.rename(
    columns={
        "OfficerID": "person_nbr",
        "EmployingEntityName": "agency_name",
        "HireDate": "start_date",
        "EndDate": "end_date",
        "RankDesc": "rank",
...
df_merged["agency_name"] = (
    df_merged["agency_name"].str.replace(".", "", regex=False).str.upper()
)


# Fix formatting of dates
df_merged["start_date"] = pd.to_datetime(
    df_merged["start_date"].astype(str), errors="coerce"
).dt.strftime("%Y-%m-%d")
df_merged["end_date"] = pd.to_datetime(
    df_merged["end_date"].astype(str), errors="coerce"
).dt.strftime("%Y-%m-%d")

# Add offense information
df_merged["offense"] = df_merged["DecertificationReason"]
df_merged["separation_reason"] = None
df_merged["separation_reason"][~(df_merged["DecertificationDate"].isna())] = (
    "decertified"
)
df_merged["separation_reason"][df_merged["DecertifiedByDCJS"] == "Yes"] = (
    "decertified by DCJS"
)
...
# Drop duplicates
keys = ['person_nbr', 'full_name', 'agency_name', 'start_date', 'end_date', 'offense', 'separation_reason']
