...
if len(out) > 0:
    ...
    if has_upper > 0 or has_whitespace > 0:
        record("person_nbr_format", "FAIL",
               f"{has_upper} rows with uppercase, {has_whitespace} rows with whitespace")
    else:
        record("person_nbr_format", "PASS", "All person_nbr lowercase and trimmed")
...
if len(out) > 0:
    ...
    if empty_start > 0:
        record("start_date_not_empty", "FAIL", f"{empty_start} rows with empty start_date")
    else:
...
if len(out) > 0:
    invalid_starts = out["start_date"].fillna("").apply(
        lambda x: bool(x) and not DATE_RE.match(x)
    ).sum()
    invalid_ends = out["end_date"].fillna("").apply(
        lambda x: bool(x) and not DATE_RE.match(x)
    ).sum()
    if invalid_starts > 0 or invalid_ends > 0:
        record("date_format", "FAIL",
               f"{invalid_starts} invalid start_date, {invalid_ends} invalid end_date")
    else:
        record("date_format", "PASS", "All non-empty dates in YYYY-MM-DD format")

# ---------------------------------------------------------------------------
# Check: no fully duplicate rows
# ---------------------------------------------------------------------------
if len(out) > 0:
    dupes = out.duplicated(subset=["person_nbr", "agency_name", "start_date"]).sum()
    if dupes > 0:
        record("no_duplicates", "WARN", f"{dupes} duplicate (person_nbr, agency_name, start_date) rows")
    else:
...
# ---------------------------------------------------------------------------
if len(out) > 0:
    code_prefix = out["agency_name"].fillna("").str.match(r"^[A-Z]\d{4}\s").sum()
    if code_prefix > 0:
        record("agency_no_code_prefix", "WARN",
               f"{code_prefix} agency names appear to have code prefixes")
    else:
        record("agency_no_code_prefix", "PASS", "No agency code prefixes detected")
...
if has_groundtruth:
    ...
    row_status = "PASS" if row_diff_pct <= 5 else "WARN" if row_diff_pct <= 15 else "FAIL"
    record("row_count", row_status,
    ...
    leo_status = "PASS" if leo_diff <= 5 else "WARN" if leo_diff <= 15 else "FAIL"
    corr_status = "PASS" if corr_diff <= 5 else "WARN" if corr_diff <= 15 else "FAIL"

    record("leo_row_count", leo_status,
           f"LEO Output={len(out_leo)}, GT={len(gt_leo)}, diff={leo_diff:.1f}%")
    record("corrections_row_count", corr_status,
    ...
    matched = out_leo_sample.merge(
        gt_leo_upper[["person_nbr_lower", "start_date", "agency_name"]].rename(
            columns={"person_nbr_lower": "person_nbr", "agency_name": "gt_agency"}
        ),
        on=["person_nbr", "start_date"],
        how="inner",
    )
    if len(matched) > 0:
        agency_match_pct = (matched["agency_name"] == matched["gt_agency"]).mean() * 100
        agency_status = "PASS" if agency_match_pct >= 90 else "WARN" if agency_match_pct >= 75 else "FAIL"
        record("agency_name_accuracy", agency_status,
               f"{agency_match_pct:.1f}% agency names match groundtruth "
               f"({len(matched)} matched rows checked)")
    else:
        record("agency_name_accuracy", "WARN", "Could not match rows for agency spot-check")
    ...
    if len(matched) > 0:
        ...
        fn_match = (
            ...
            matched["gt_agency"].apply(lambda x: "").str.upper()  # placeholder
        )
        # Compare with GT first_name
        gt_names = gt_leo_upper[["person_nbr_lower", "start_date", "first_name", "last_name"]].rename(
            columns={"person_nbr_lower": "person_nbr", "first_name": "gt_first", "last_name": "gt_last"}
        )
        matched2 = out_leo_sample.merge(gt_names, on=["person_nbr", "start_date"], how="inner")
        if len(matched2) > 0:
            fn_acc = (matched2["first_name"].str.strip().str.upper() ==
                      matched2["gt_first"].str.strip().str.upper()).mean() * 100
            ln_acc = (matched2["last_name"].str.strip().str.upper() ==
                      matched2["gt_last"].str.strip().str.upper()).mean() * 100
            name_status = "PASS" if min(fn_acc, ln_acc) >= 90 else "WARN" if min(fn_acc, ln_acc) >= 75 else "FAIL"
            record("name_accuracy", name_status,
                   f"first_name {fn_acc:.1f}%, last_name {ln_acc:.1f}% match groundtruth")
        else:
            record("name_accuracy", "WARN", "Could not match rows for name spot-check")

    # Spot-check: separation_reason
    if "separation_reason" in out.columns and "separation_reason" in gt.columns:
        gt_sep = gt_leo_upper[["person_nbr_lower", "start_date", "separation_reason"]].rename(
            columns={"person_nbr_lower": "person_nbr", "separation_reason": "gt_sep"}
        )
        matched3 = out_leo_sample.merge(gt_sep, on=["person_nbr", "start_date"], how="inner")
        if len(matched3) > 0:
            ...
            if len(both_filled) > 0:
                ...
                sep_status = "PASS" if sep_acc >= 90 else "WARN" if sep_acc >= 75 else "FAIL"
                record("separation_reason_accuracy", sep_status,
    ...
    record("corrections_agency_format", corr_agency_status,
           f"{corr_agency_pct:.1f}% corrections agency names match 'NNN: NAME' format")

else:
    record("groundtruth", "WARN", "No groundtruth found — skipping comparison checks")
...
if len(out) > 0:
    ...
    if "type" in out.columns:
        ...
        if bad_types > 0:
            record("type_column", "WARN", f"{bad_types} rows with unexpected type values")
        else:
            record("type_column", "PASS", f"All type values are valid: {valid_types}")

    # No person_nbr that is NaN / empty
    empty_nbr = (out["person_nbr"].fillna("") == "").sum()
    if empty_nbr > 0:
        record("person_nbr_not_empty", "FAIL", f"{empty_nbr} rows with empty person_nbr")
    else:
        record("person_nbr_not_empty", "PASS", "No empty person_nbr values")

    # Summary statistics
    leo_pct = out["person_nbr"].str.contains("-", na=False).mean() * 100
    record("data_composition", "PASS",
...
print(f"Wrote judge_report.json")
print(f"\nOverall: {overall}")
