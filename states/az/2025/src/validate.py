...
if not os.path.exists(OUTPUT_PATH):
    ...
    with open(os.path.join(OUTPUT_DIR, "judge_report.md"), "w") as f:
        f.write("# AZ 2025 Validation Report\n\n**FAIL** — output file not found.\n")
...
if missing_cols:
    ...
    check("required_columns", "PASS", f"All required columns present: {REQUIRED_COLS}")

# ---------------------------------------------------------------------------
# person_nbr format
# ---------------------------------------------------------------------------

bad_pnbr = df[df['person_nbr'].isna() | (df['person_nbr'].str.strip() == '') |
...
if len(bad_pnbr) > 0:
    ...
    check("person_nbr_format", "PASS", "All person_nbr are lowercase and non-empty")
...
if len(empty_start) > 0:
    check("start_date_nonempty", "FAIL",
...
if any(b > 0 for b in bad_sentinels):
    check("no_date_sentinels", "FAIL", f"Found invalid date sentinels: {bad_sentinels}")
else:
...
if code_prefix.sum() > 0:
    check("agency_no_code_prefix", "FAIL",
...
if has_groundtruth:
    ...
    if shared_ids:
        ...
        gt_sub = gt[gt['person_nbr'].astype(str).str.strip().str.lower().isin(shared_ids)].copy()
        gt_sub['person_nbr'] = gt_sub['person_nbr'].astype(str).str.strip().str.lower()
        ...
        def normalize_date(s):
            if pd.isna(s) or str(s).strip() == '':
                return ''
            try:
                return pd.to_datetime(str(s).strip(), errors='coerce').strftime('%Y-%m-%d')
            except Exception:
                return ''
        ...
        merged = out_sub.merge(
            ...
            on='person_nbr', how='inner'
        )
        if len(merged) > 0:
            start_match = (merged['start_date'] == merged['start_date_norm']).mean()
            end_match = (
                (merged['end_date'].fillna('') == merged['end_date_norm'].fillna(''))
            ).mean()
            if start_match >= 0.5:
                check("date_value_spot_check", "PASS",
                      f"start_date match: {start_match:.1%}, end_date match: {end_match:.1%}")
            else:
                check("date_value_spot_check", "WARN",
                      f"start_date match: {start_match:.1%}, end_date match: {end_match:.1%}")

else:
    check("groundtruth", "WARN", "No groundtruth directory found — skipping comparison checks")
