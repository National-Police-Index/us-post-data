...
before = len(df)
df = df[df['start_date'] != '']
print(f"  Dropped {before - len(df):,} rows with empty start_date; {len(df):,} remain")
...
def clean_agency_name(name):
    ...
    s = re.sub(r'\s*/\s*(INACTIVE|ACTIVE|CLOSED|RETIRED).*$', '', s, flags=re.IGNORECASE)
...
if 'current_certificate_status' in df.columns:
    ...
    df['employment_status'] = df['employment_status'].fillna('').astype(str).str.strip()
...
output_cols = [
    'person_nbr', 'first_name', 'last_name', 'middle_name', 'full_name',
    'agency_name', 'rank', 'start_date', 'end_date', 'employment_status',
]
