...
if not os.path.exists(idx_path):
    ...
    with open(os.path.join(OUTPUT_DIR, "judge_report.json"), "w") as f:
        json.dump(
            {"overall": FAIL, "has_groundtruth": False, "checks": checks}, f
        )
    sys.exit(1)
...
if disc_exists:
    ...
    disc_df = None
    add_check(
        "discipline_index_exists",
        WARN,
        f"Discipline index not found: {disc_path}",
    )
...
if missing_emp:
    ...
    add_check(
        ...
        PASS,
        f"All required columns present: {REQUIRED_EMP_COLS}",
    )
...
if disc_df is not None:
    ...
    if missing_disc:
        add_check(
            "disc_required_columns", FAIL, f"Missing columns: {missing_disc}"
        )
    else:
        add_check(
            "disc_required_columns",
            PASS,
            "All required discipline columns present",
        )
...
if bad_empty > 0:
    add_check(
        "emp_person_nbr_nonempty",
        FAIL,
        f"{bad_empty:,} empty person_nbr values",
    )
else:
...
if bad_upper > 0:
    add_check(
        "emp_person_nbr_lowercase",
        FAIL,
        f"{bad_upper:,} person_nbr values not lowercase",
    )
else:
    add_check(
        "emp_person_nbr_lowercase", PASS, "All person_nbr values are lowercase"
    )

if bad_ws > 0:
    add_check(
        "emp_person_nbr_no_whitespace",
        WARN,
        f"{bad_ws:,} person_nbr values have leading/trailing whitespace",
    )
else:
    add_check(
        "emp_person_nbr_no_whitespace",
        PASS,
        "No leading/trailing whitespace in person_nbr",
    )
...
if ga_format_ok:
    add_check(
        "emp_person_nbr_ga_format",
        PASS,
        f"person_nbr starts with 'o' as expected (sample: {sample_pnbr[:3]})",
    )
else:
    add_check(
        "emp_person_nbr_ga_format",
        WARN,
        f"Unexpected person_nbr format (sample: {sample_pnbr[:3]})",
    )
...
if empty_start > 0:
    add_check(
        ...
        FAIL,
        f"{empty_start:,} empty start_date values",
    )
else:
    add_check("emp_start_date_nonempty", PASS, "No empty start_date values")

status, msg = check_date_col(
    emp_df, "start_date", "start_date", allow_zero=True
)
...
if disc_df is not None:
    empty_disc_start = (
        disc_df["start_date"].isna() | (disc_df["start_date"] == "")
    ).sum()
    if empty_disc_start > 0:
        add_check(
            "disc_start_date_nonempty",
            FAIL,
            f"{empty_disc_start:,} empty discipline start_date values",
        )
    else:
        add_check(
            "disc_start_date_nonempty",
            PASS,
            "No empty discipline start_date values",
        )

    # violation_date: no 0000-00-00 (filtered in clean.py)
    if "violation_date" in disc_df.columns:
        status, msg = check_date_col(
            disc_df, "violation_date", "violation_date", allow_zero=False
        )
    ...
    if "sanction_date" in disc_df.columns:
        status, msg = check_date_col(
            disc_df, "sanction_date", "sanction_date", allow_zero=True
        )
...
has_code_prefix = sum(
    1 for a in sample_agencies if str(a).startswith("G") and len(str(a)) > 5
)
if has_code_prefix >= 5:
    add_check(
        "emp_agency_has_code_prefix",
        PASS,
        f"Agency names have code prefix as expected (sample: {sample_agencies[:3]})",
    )
else:
    add_check(
        "emp_agency_has_code_prefix",
        WARN,
        f"Agency names may be missing code prefix (sample: {sample_agencies[:3]})",
    )
...
if bad_agency > 0:
    add_check(
        "emp_agency_no_noise",
        WARN,
        f"{bad_agency:,} non-agency values in agency_name",
    )
else:
    add_check(
        "emp_agency_no_noise", PASS, "No non-agency noise values in agency_name"
    )
...
if emp_dupes > 0:
    add_check(
        "emp_no_duplicates",
        WARN,
        f"{emp_dupes:,} duplicate rows (person_nbr + agency_name + start_date)",
    )
else:
    add_check(
        "emp_no_duplicates", PASS, "No duplicate rows in employment index"
    )

if disc_df is not None and "case_id" in disc_df.columns:
    disc_dupes = disc_df.duplicated(
    ...
    if disc_dupes > 0:
        add_check(
            "disc_no_duplicates",
            WARN,
            f"{disc_dupes:,} duplicate rows in discipline index",
        )
    else:
        add_check(
            "disc_no_duplicates", PASS, "No duplicate rows in discipline index"
        )
...
if has_groundtruth:
    ...
    gt_disc = (
        pd.read_csv(gt_disc_path, dtype=str)
        if os.path.exists(gt_disc_path)
        else None
    )
    ...
    if not extra and not missing:
        add_check(
            "emp_column_match",
            PASS,
            "Output columns match ground truth exactly",
        )
    else:
        msg_parts = []
        if missing:
    ...
    match_rate = (
        len(shared_idx) / len(gt_sample) * 100 if len(gt_sample) > 0 else 0
    )
    if match_rate >= 90:
        add_check(
            "emp_value_spot_check",
            PASS,
            f"{match_rate:.0f}% of sample rows match GT on (person_nbr, agency, start_date)",
        )
    elif match_rate >= 70:
        add_check(
            "emp_value_spot_check",
            WARN,
            f"Only {match_rate:.0f}% of sample rows match GT on key columns",
        )
    else:
        add_check(
            "emp_value_spot_check",
            WARN,
            f"Low match rate: {match_rate:.0f}% of sample rows match GT",
        )

    # Name format check: GT has lowercase last_name
    if "last_name" in gt_emp.columns and "last_name" in emp_df.columns:
        gt_upper = (
            gt_emp["last_name"].dropna()
            != gt_emp["last_name"].dropna().str.lower()
        ).sum()
        out_upper = (
            emp_df["last_name"].dropna()
            != emp_df["last_name"].dropna().str.lower()
        ).sum()
        if out_upper == 0:
            add_check(
                "emp_last_name_lowercase",
                PASS,
                "last_name is lowercase (matches GT convention)",
            )
        else:
            add_check(
                "emp_last_name_lowercase",
                WARN,
                f"{out_upper:,} last_name values are not lowercase",
            )

    # Discipline value spot-check
    if gt_disc is not None and disc_df is not None:
        disc_key = ["case_id", "person_nbr", "violation", "sanction"]
        if all(c in disc_df.columns for c in disc_key) and all(
            c in gt_disc.columns for c in disc_key
        ):
            gt_disc_keys = set(
                zip(
                    gt_disc["case_id"],
    ...
    add_check(
        "no_groundtruth",
        WARN,
        "Ground truth not available; skipping comparison checks",
    )
...
def write_reports(checks_list, overall_status, has_gt):
    ...
    for c in checks_list:
        ...
        md_lines.append(
            f"| {c['name']} | {symbol} {c['status']} | {c['message']} |"
        )
