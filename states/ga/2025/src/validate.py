...
# ---------------------------------------------------------------------------
DISC_REQUIRED = ['person_nbr', 'first_name', 'last_name',
                 'agency_name', 'start_date', 'end_date',
...
if missing_disc:
    ...
    check("disc_schema", "PASS", f"All required columns present")
...
if bad_upper == 0 and bad_ws == 0:
    check("person_nbr_format", "PASS", "All person_nbr are lowercase and trimmed")
elif bad_upper + bad_ws < 10:
    check("person_nbr_format", "WARN",
          f"{bad_upper} uppercase, {bad_ws} whitespace issues in person_nbr")
else:
    check("person_nbr_format", "FAIL",
          f"{bad_upper} uppercase, {bad_ws} whitespace issues in person_nbr")
...
def check_dates(df, col, label):
    ...
    bad = (~valid).sum()
    if bad == 0:
        check(f"{label}_{col}_format", "PASS", f"All {col} values are valid date strings")
    else:
        sample = vals[~valid].head(3).tolist()
        check(f"{label}_{col}_format", "WARN",
...
if empty_starts_emp == 0:
    check("emp_start_date_nonempty", "PASS", "No empty start_date values in employment index")
else:
    check("emp_start_date_nonempty", "WARN",
...
if empty_starts_disc == 0:
    check("disc_start_date_nonempty", "PASS", "No empty start_date in discipline index")
else:
    check("disc_start_date_nonempty", "WARN",
          f"{empty_starts_disc} empty start_date in discipline index")

# ---------------------------------------------------------------------------
# Duplicate check
# ---------------------------------------------------------------------------
emp_dupes = emp.duplicated(subset=['person_nbr', 'agency_name', 'start_date']).sum()
if emp_dupes == 0:
    check("emp_no_duplicates", "PASS", "No duplicate (person_nbr, agency_name, start_date) rows")
elif emp_dupes < 100:
    check("emp_no_duplicates", "WARN", f"{emp_dupes} duplicate rows in employment index")
else:
    check("emp_no_duplicates", "FAIL", f"{emp_dupes} duplicate rows in employment index")

disc_dupes = disc.duplicated(subset=['case_id', 'person_nbr', 'violation', 'sanction']).sum()
if disc_dupes == 0:
    check("disc_no_duplicates", "PASS", "No duplicate (case_id, person_nbr, violation, sanction) rows")
elif disc_dupes < 100:
    check("disc_no_duplicates", "WARN", f"{disc_dupes} duplicates in discipline index")
else:
    check("disc_no_duplicates", "FAIL", f"{disc_dupes} duplicates in discipline index")
...
if has_groundtruth:
    ...
    if disc_pct <= 20:
        check("gt_disc_row_count", "PASS" if disc_pct <= 5 else "WARN",
              f"Discipline rows: {len(disc):,} vs GT {len(gt_disc):,} ({disc_pct:.1f}% diff)")
    else:
        check("gt_disc_row_count", "WARN",
              f"Discipline rows: {len(disc):,} vs GT {len(gt_disc):,} ({disc_pct:.1f}% diff) — "
    ...
    for pid in SAMPLE_PERSONS:
        ...
        if len(gt_rows) == 0:
            continue
        if len(out_rows) == 0:
            check(f"spot_{pid}", "WARN", f"Person {pid} in GT but not in output")
            continue
    ...
    has_bare_code = any(a.strip().upper().startswith('G') and ' ' not in a
                        for a in sample_agencies_emp)
    if not has_bare_code:
        check("emp_agency_name_format", "PASS",
              "agency_name values look like full names (not bare codes)")
    else:
        check("emp_agency_name_format", "WARN",
...
def _write_report(checks, has_gt):
    ...
    for i, c in enumerate(checks, 1):
        ...
        lines.append(f"| {i} | {c['name']} | {icon} {c['status']} | {c['detail']} |")
