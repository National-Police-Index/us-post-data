...
emp = emp[emp['person_nbr'].notna() & (emp['person_nbr'] != '') & (emp['person_nbr'] != 'nan')]
...
demo['year_of_birth'] = demo['year_of_birth'].astype(str).str.strip().replace('nan', '')
...
# ---------------------------------------------------------------------------
merged = emp.merge(
    demo[['person_nbr', 'last_name', 'first_name', 'middle_name',
...
# Fill NaN strings
for col in ['last_name', 'first_name', 'middle_name', 'suffix', 'year_of_birth', 'race', 'sex']:
...
inv  = pd.read_csv(os.path.join(INPUT_DIR, 'officer_investigations.csv'), dtype=str)

print(f"  violations: {len(viol):,}  sanctions: {len(sanc):,}  investigations: {len(inv):,}")
...
print(f"  Violations with valid date: {len(viol_valid):,} (dropped {len(viol)-len(viol_valid):,} with 0000-00-00)")

# Title-case violation and sanction strings (matches GT)
viol_valid['violation'] = viol_valid['violation'].astype(str).apply(title_case_str)
...
emp_ctx = emp[['person_nbr', 'agency_name', 'rank', 'start_date', 'end_date']].copy()
emp_ctx['agency_name_lc'] = emp_ctx['agency_name'].str.lower()

# Deduplicate employment by (person, agency, start_date)
emp_dedup = emp_ctx.drop_duplicates(subset=['person_nbr', 'agency_name_lc', 'start_date'])

# Join discipline to employment on (person_nbr, agency from investigation)
disc_emp = disc.merge(
    emp_dedup[['person_nbr', 'agency_name_lc', 'rank', 'start_date', 'end_date']].rename(columns={'agency_name_lc': 'inv_agency'}),
...
def score_period(row):
    ...
    try:
        ...
        s = pd.to_datetime(sd) if sd != '0000-00-00' else pd.Timestamp('1900-01-01')
...
disc_emp['_min_score'] = disc_emp.groupby(['case_id', 'person_nbr', 'violation', 'sanction'])['_score'].transform('min')
...
disc_final['rank'] = disc_final['rank'].astype(str).str.lower().replace('nan', '')
...
print(f"  Dropped {before - len(disc_final):,} discipline rows with no employment match")
...
empty_start = (ga_index['start_date'].isna() | (ga_index['start_date'] == '')).sum()
assert empty_start == 0, f"Employment index has {empty_start} empty start_date rows!"

empty_disc_start = (ga_disc['start_date'].isna() | (ga_disc['start_date'] == '')).sum()
assert empty_disc_start == 0, f"Discipline index has {empty_disc_start} empty start_date rows!"
