...
OD_COLS = ['person_nbr', 'last_name', 'first_name', 'middle_name', 'suffix',
           'year_of_birth', 'race', 'sex', 'full_name']
...
emp_merged = emp_merged.drop_duplicates(subset=['person_nbr', 'agency_name', 'start_date'])
...
def clean_case_series(series):
    ...
    return numeric.apply(
        lambda x: '' if pd.isna(x) else str(int(x)).zfill(10)
    )
...
score = np.where(no_data, 9_999 * NS_PER_DAY,
        np.where(ongoing_good | bounded_good, 0,
        np.where(before_start, sd_ns - vd_ns,
        np.where(after_end, vd_ns - ed_ns,
                 9_999 * NS_PER_DAY))))
...
for col in ['person_nbr', 'first_name', 'last_name', 'agency_name', 'start_date', 'end_date']:
    assert col in emp_out.columns, f"Missing required column in employment index: {col}"
    assert col in disc_out.columns, f"Missing required column in discipline index: {col}"

assert (emp_out['person_nbr'].str.lower() == emp_out['person_nbr']).all(), \
    "person_nbr has uppercase values"
assert not emp_out['person_nbr'].str.contains(r'^\s|\s$', regex=True, na=False).any(), \
    "person_nbr has leading/trailing whitespace"

empty_starts = (emp_out['start_date'] == '').sum()
if empty_starts > 0:
    print(f"  WARNING: {empty_starts} rows with empty start_date in employment index")
