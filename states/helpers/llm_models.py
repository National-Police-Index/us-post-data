...
# Available models
AVAILABLE_MODELS = ["gpt-4.1-mini", "gpt-4.1", "gpt-4.1-mini-2025-04-14"]
...
class LLM:
    ...
    def _run_raw_inference(
        ...
        if model_to_use not in AVAILABLE_MODELS:
            raise ValueError(
                f"Model {model_to_use} not available. Available: {AVAILABLE_MODELS}"
            )
    ...
    def test_connection(self):
        ...
        try:
            ...
            logger.info(
                f"LLM Test Response: {response.message} (Status: {response.status})"
            )
            return True
...
def run_structured_inference(
    prompt: str, response_model: Type[T], **kwargs
) -> T:
